...
class RCModel(object):
    ...
    def _compute_loss(self):
        ...
        def sparse_nll_loss(probs, labels, epsilon=1e-9, scope=None):
            ...
            with tf.name_scope(scope, "log_loss"):
                ...
                losses = -1* tf.reduce_sum(labels * tf.log(probs + epsilon), 1)
            return losses
    ...
    def demo(self,batch,out_path=None):
        self.logger.info('start demo')
        feed_dict = {self.p: batch['passage_token_ids'],
             self.q: batch['question_token_ids'],
             self.p_length: batch['passage_length'],
             self.q_length: batch['question_length'],
             self.start_label:[0 for _ in range(len(batch['passage_length']))],
             self.dropout_keep_prob: 1.0}
        start_probs, end_probs= self.sess.run([self.start_probs,self.end_probs], feed_dict)
        
        out_f = None
        if out_path is not None:
             out_f = open(out_path,'w',encoding='utf-8')

        for question,passage,start_prob, end_prob in zip(batch['questions'],batch['passages'],start_probs, end_probs):
            best_span, max_probs= self.find_best_answer_for_passage(start_prob,end_prob)
            best_answer =''.join(passage[best_span[0]: best_span[1] + 1]) 
            print('best answer')
            print(best_answer)
            if out_f is not None:
                out_f.write('Q:\n%s\n====\n%s\n===\nA:\n%s\n\n'%(question,passage,best_answer))
